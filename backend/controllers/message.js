...
const sendMessage = async(req, res) => {
    try {
        ...
        return res.status(500).json({ message: 'Error while sending message', error: error.message });
    }
}
...
const getMessage = async (req, res) => {
    try {
        const messages = await Message.find({ $or: [{ sender: req.senderId, recipient: req.params.receiverId },
            { sender: req.params.receiverId, recipient: req.senderId }] }
        ). populate('sender', 'name').populate('recipient', 'name').sort({ createdAt: 1 });
...
const deleteMessage = async (req, res) => {
    try {
        ...
        if (existingMessage.sender.toString() !== req.senderId && existingMessage.recipient.toString() !== req.senderId) {
        ...
        return res.status(500).json({ message: 'Error while deleting message', error: error.message });
    }
}

module.exports = {
    sendMessage, getMessage, patchMessage, deleteMessage
