...
//   try {
//     const { recipient, sender, notificationType, message } = req.body;
...
const getNotification = async (req, res) => {
  ...
  try {
    const notifications = await Notification.find({ recipient: req.senderId }).populate('sender', 'name').sort({ createdAt: -1}); 

    res.status(201).json({ message: "Notifications fetched successfully", data: notifications });

  } catch (error) {
    res
      .status(500)
      .json({ message: "Error while fetching notifications ", error: error.message });
  }
};
...
const  updateNotification = async (req, res) => {
  try {
    ...
    await Notification.updateMany({ recipient: req.senderId, isRead: false }, { isRead: true });
